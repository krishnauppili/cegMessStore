<script>
   ...
   function submit_update(){
      ...
      function get_report(){
            ...
            $(report).html("");
            $.ajax({
                  url : 'get_mess_vegetable_consumption_report/'+messName+'/'+from+'/'+to,
                  type : 'GET'  ,
                  dataType : 'json',
                  success : function(data){
                        if(data.vegetableNames.length  == 0){
                              dataToPrint += '<tr>'+
                                 '<th>'+
                              ...
                              dataToPrint += '<tr>'+                            
                                 ...
                                 '<th>'+
                                    ...
                                    '</th>'+
                                 '<th>'+
                                    '<span class="blue-text">Proposed Rate</span>'+
                                    '</th>'+
                                 '<th>'+
                                    '<span class="blue-text">Competitive Rate</span>'+
                                    '</th>'+
                                 '<th>'+
                              ...
                              $(data.vegetableNames).each(function(index){
                                    var editData = {};
                                    editData["suppliedDate"] = data.suppliedDate[index];
                                    editData["itemName"] = encodeURIComponent(data.vegetableNames[index]);
                                    editData["messName"] = messName;
                                    editData["latestRate"] = data.proposedRate[index];
                                    editData["quantitySupplied"] = data.quantitySupplied[index];

                                    var jsonEdit = JSON.stringify(editData);
                                    console.log(jsonEdit);
                                    var itemName = encodeURIComponent(data.vegetableNames[index]);
                                    ...
                                    dataToPrint += '<tr>'+
                                       '<th>'+
                                          ...
                                          '</th>'+
                                       '<th>'+
                                          data.vegetableNames[index]+
                                          '</th>'+
                                       '<th>'+
                                          data.quantitySupplied[index]+
                                          '</th>'+
                                       '<th>'+
                                          data.proposedRate[index]+
                                          '</th>'+
                                       '<th>'+
                                          data.actualRate[index]+
                                          '</th>'+
                                       '<th>'+
                                          data.amount[index]+
                                          '</th>';
      ...
      <form name="selection" method="post"  action="mess_vegetable_consumption"> 
         <div class="row">
