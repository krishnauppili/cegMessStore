<script>
   ...
   function get_vegetable_transactions(){
         var from = encodeURIComponent($("#from").val());
         var to = encodeURIComponent($("#to").val())
         $.ajax({
               url : 'get_edit_vegetable_transactions/'+from+'/'+to,
               type: 'GET',
               dataType: 'json',
               success : function(data){
                     ...
                     var jsonObj = data;
                     console.log(data);
                     var htmlContents = '<form name="abstract" id ="abstract" action="edit_vegetable_transaction_form" method="post">';	
                        htmlContents += '<ul class="collapsible" data-collapsible="accordion" style = "width: 1200px">';
                           for (i = 0; i < jsonObj.length; i++) {
                                 var t_id = jsonObj[i].t_id;
                                 var vendor_name = jsonObj[i].vendorName;
                                 var mess_name = jsonObj[i].messName;
                                 var t_date = jsonObj[i].receivedDate;
                                 ...
                                 htmlContents +=  '<div class = "collapsible-body">'+
                                    '<div class= "row">'+
                                       '<div class = "col s12 offset-s1" style = "width: 500px" >'+
                                    ...
                                    for(j=0;j<items.length;j++){
                                          var temp = {};
                                          temp["item_name"] = items[j].itemName.split(' ').join('_');
                                          temp["t_id"] = items[j].t_id;
                                          temp["quantity"] = items[j].quantityReceived;
                                          temp["amount"] = items[j].amount;
                                          temp["rate"] = items[j].proposedRate;
      ...
      function deleteItemOrder(details)
      {
         ...
         if(confirm('Do you really want to delete this transaction ? This cannot be undone' )) {
               $.ajax({
                     type: "POST",
                     url: "<?php echo base_url()."orders/delete_vegetable_transaction_details";?>",
                     cache: false,
                     data: {'t_id' : t_id},
                     dataType: 'html',
   ...
   <div class="row">
      <div class='col s6'>
         <a href="javascript:get_vegetable_transactions();" class="btn waves-effect waves-light" id="getButton">
            Get Report
         </a>
      </div>
   ...
   <div class="modal fade in" id="exampleModal1" tabindex="-1" role="dialog" aria-labelledby="memberModalLabel" aria-hidden="false">
      <div class="modal-content">
         <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
            <h4 class="modal-title" id="memberModalLabel">Edit Order Primary Details</h4>
         </div>
         <div class="modal-body">
         </div>
      </div>
...
<script>
   $('#exampleModal').on('show.bs.modal', function (event) {
         ...
         $.ajax({
               type: "POST",
               url: "<?php echo base_url()."orders/edit_vegetable_transactions_form";?>",
               cache: false,
               data: recipient,
               dataType: 'html',
